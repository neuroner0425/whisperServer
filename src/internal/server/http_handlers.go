...
func buildRouteHandlers(svc appServices, spaIndex echo.HandlerFunc) httptransport.Handlers {
	...
	return httptransport.Handlers{
		...
		APIRefineJob:       httptransport.JobControlHandlers{CurrentUserOrUnauthorized: transportCurrentUserOrUnauthorized, GetJob: svc.runtime.GetJob, SetJobFields: svc.runtime.SetJobFields, BlobSvc: svc.blobSvc, EnqueueRefine: svc.runtime.EnqueueRefine, HasGeminiConfigured: svc.canGemini, StatusCompleted: statusCompleted, StatusRefiningPending: statusRefiningPending}.Refine(),
