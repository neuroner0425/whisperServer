...
func newAppServices() appServices {
	...
	blobSvc := service.NewJobBlobService(service.JobBlobServiceDeps{
		...
		BlobKindRefinedTimeline:    store.BlobKindRefinedTimeline,
	...
	geminiRt := intgemini.New(intgemini.Config{
		...
		RefineTimelineSystemPromptPath: filepath.Join(projectRoot, "docs", "prompts", "refine_timeline_system_prompt.md"),
...
func newAppWorker(blobSvc *service.JobBlobService, geminiRt *intgemini.Runtime, whisperRt *intwhisper.Runtime) *worker.Worker {
	...
	return worker.New(worker.Config{
		...
		RequeueVADFallbackOnStartup: startupVADFallbackRequeueEnabled(),
		TmpFolder:                   tmpFolder,
		ModelDir:                    modelDir,
		WhisperCLI:                  whisperCLI,
		JobTimeoutSec:               jobTimeoutSec,
		...
		PolishTranscriptTimeline:      geminiRt.PolishTranscriptTimeline,
		StructureTranscriptParagraphs: geminiRt.StructureTranscriptParagraphs,
